        ...
        if((cpid=fork())==0)
        {
            ...
            while(1)
            {
                ...
                while(1)
                {
                    ...
                    {
                        char *Faculty = "Enter the Choice :\n 1) Add New Course\n 2) Remove Offered Course\n 3) View Enrollments In Courses\n 4) Update Course Details\n 5) Password Change\n 6) Exit\n";
                        send(new_socket,Faculty,strlen(Faculty),0);
                        w = recv(new_socket,ans1,strlen(ans1),0);
                        if(ans1[0]=='6')
                            break;
                    }
